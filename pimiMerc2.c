...
int venda(){
	...
	do{	
		...
		Venda *pNovaVenda = (Venda*) malloc(sizeof(Venda));
    	pNovaVenda->vendaProximo = NULL;
    	
		while(pAux != NULL){
			if(pAux->iCodigo == cCodProduto){
				do{
					printf("Nome Produto: %s \n", pAux->cvNome);
					printf("Entre com a quantidade: ");
					scanf("%d", &iQuantidade);
					pNovaVenda->cCodProduto = pAux->iCodigo;
					pNovaVenda->dTotalCompra += iQuantidade * pAux->fValor;
					printf("Deseja adicionar mais um item? (s/n): ");
					scanf(" %c", &finalizar);
				}while(finalizar != 'n');
			
				if(vVendaInicial == NULL){
					vVendaInicial = pNovaVenda;
				}else{
					while(vAux->vendaProximo != NULL){
						vAux = vAux->vendaProximo;
					}
					vAux->vendaProximo = pNovaVenda;
				}
				iNaoEncontrado++;
			}
			pAux = pAux->pProximo;
		...
		printf("finalizar comprar (s/n): ");
		scanf(" %c", &finalizar);
		while(vAux != NULL){
			printf("Codigo: %d\nTotal: %.2f", vAux->cCodProduto, vAux->dTotalCompra);
			vAux = vAux->vendaProximo;
		}
	}while(finalizar!='s');
	printf("Nao sei\n");
